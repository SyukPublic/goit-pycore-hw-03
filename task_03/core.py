...
PATTERN_CLEANING_NON_DIGITS = re.compile(r'\D+')

# Pattern of the phone number ungroup by country code, carrier code, and number
PATTERN_PHONE_NUMBER_UNGROUP = re.compile(r'(\+?\d{,3})(\d{2})(\d{7})')
# Pattern of the phone number ungroup by country code (without '+'), carrier code, and number
PATTERN_PHONE_NUMBER_UNGROUP_ALT = re.compile(r'(\d{,3})(\d{2})(\d{7})')

# Pattern of the country code verification
PATTERN_COUNTRY_CODE_VERIFY = re.compile(r'^\+\d{3}$')


def normalize_phone(phone_number: typing.Optional[str]) -> str:
    """The function generates a list of unique random numbers of the specified length within the given range.

    :param phone_number: phone number string (str, optional)
    :return: normalized phone number string (str)
    """
    try:
        ...
        #  Group phone number from country code, carrier code, and number, and return
...
def normalize_phone_alt(phone_number: typing.Optional[str]) -> str:
    """The function generates a list of unique random numbers of the specified length within the given range.
    ...
    try:
        ...
        # Clean phone number from garbage symbols
        phone_number = re.sub(PATTERN_CLEANING_NON_DIGITS, '', phone_number)
        ...
        # Ungroup phone number by country code, carrier code, and number
        match = re.search(PATTERN_PHONE_NUMBER_UNGROUP_ALT, phone_number)
        if match is not None:
            country_code, carrier_code, number = match.groups()
            match len(country_code):
                case 0:
